...
- Two `Texture2DRD` resources saved as standalone `.tres` files: one assigned to the compositor's `SegmentTexture` and each consumer material's `segments` uniform, the other to the compositor's `HeightTexture` and each consumer's `height_buffer` uniform. These are the shared channels: the compositor sets their RIDs, every consumer samples them. The compositor's `BufferSize` is set to the SubViewport's render size so the shared textures are allocated at their final size up front (no first-render resize; see the Texture2Drd gotchas).
...
- `scenes/DemoTerrain.tscn`: a single baked terrain + top-down camera + a full-screen `topographic.gdshader` `ColorRect`, driven by `scripts/DemoTerrain.cs`. It is the one-stop testing scene: view mode (no arg) renders the schematic torture terrain (`torture_heightmap.exr`, the stress rig described below) for the editor preview and Play, while `-- banner` and `-- presets` swap the terrain material back to the smooth `banner_heightmap.exr` to regenerate the README screenshots (only the gradient changes between shots). The swap is one `SetShaderParameter("heightmap", ...)` in `DemoTerrain.cs` via its `Terrain` export.
...
- The topographic effect lives only on the orthographic map camera (via the `Compositor` on `TopDownCamera`). It must never affect the main gameplay view or the editor's main 3D viewport. It may render into its own map ColorRects in the editor for authoring preview (the consumer's inspector-bound inputs draw at author time), confined to the map's own SubViewport and Control nodes.
...
- A consumer must not draw while sampling these textures before the producer's first render: with a `Texture2Drd`'s RID not yet live, the canvas shader cannot supply its sampler uniform and the draw fails with "Uniforms were never supplied for set (1)". Two defenses: (1) the compositor exposes `HasProduced` (set at the end of its first render callback) and the always-visible minimap stays hidden until it is true, so the only startup consumer never draws before the producer runs (the world map is hidden until opened, well after production); (2) the compositor arms a placeholder for each `Texture2DRD` as soon as its property is set, so the wrapper RID is never empty, replaced with the real data on the first render. The shader treats `seg.x <= 0` as "no contour", which also covers the zero-initialized placeholder and any out-of-range fetch.
- Reassigning a `Texture2DRD`'s RID (placeholder to real size on the first render) invalidates a consumer's cached uniform set for one frame and trips "set (1)" (Godot #118292). The `HasProduced` gate hides runtime consumers past that frame, but in the editor no gate runs, so opening a consumer scene cold would log it once. The compositor's `BufferSize` export avoids the reassignment entirely: set it to the map SubViewport's render size and the placeholders are allocated at the final size, so the first render finds the size unchanged and never resizes (no RID swap, no editor warning). Export order is not guaranteed, so the `BufferSize` setter re-arms any placeholder a texture setter already created at the 1x1 default. Left at `(0,0)`, a 1x1 placeholder is used and the one-time cold-open warning returns.
- When replacing an RD texture (placeholder to real size), create the new texture and set `TextureRdRid` to it BEFORE freeing the old RD texture. The `Texture2Drd` wrapper still references the old RID through its internal RenderingServer texture, so freeing the old RD texture first makes the setter operate on a freed RID and crashes with "Attempted to free invalid ID". On teardown, clear each wrapper (`TextureRdRid = new Rid()`) first, then free the RD texture.

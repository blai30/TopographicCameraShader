...
- Minor-line legibility fade: minor lines fade out where their estimated on-screen spacing collapses, so dense minors do not moire when zoomed out and do not smear into a solid band on steep slopes. The spacing is `contour_interval / fwidth(h)` (the height change per screen pixel), so a single quantity covers both causes at once: zooming out makes `buffer_uv` move faster per pixel, and a steep slope makes `h` move faster per UV. The fade is a `smoothstep` on minor line alpha only; major (index) lines never fade and the tint is untouched. Controlled by `minor_legibility_px` (the spacing threshold, `0` = off, the default) and `minor_legibility_softness_px`. Off by default; the demo torture terrain opts in at `3`.

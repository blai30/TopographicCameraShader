...
- A `Texture2DRD` resource saved as a standalone `.tres`, assigned both to the compositor's `SegmentTexture` and to each consumer material's `segments` uniform. This one resource is the shared channel: the compositor sets its RID, every consumer samples it.
- Each consumer's `height_buffer` bound to a `ViewportTexture` of the producer's `SubViewport`.
...
- `scenes/DemoMinimap.tscn`: the `MapView` SubViewport + `TopDownCamera` + compositor, the two map `ColorRect`s (each running `topographic.gdshader` with a `Marker` child overlay), and the HUD. The shared `Texture2DRD` (`map_segment_texture.tres`) is assigned both to the compositor effect's `SegmentTexture` and to each material's `segments` uniform in the scene, and each material's `height_buffer` is a `ViewportTexture` of the `MapView` SubViewport. `MapUi` references the compositor effect (`map_view_compositor_effect.tres`) only to gate the minimap reveal on `HasProduced`.
...
- A `Camera3D` renders into its nearest ancestor `Viewport`/`SubViewport` (going toward the root), automatically and with no script. This is the only thing wiring `TopDownCamera` to the `MapView` SubViewport: there is no reference between them, just the nesting, so the camera must stay inside the SubViewport's subtree (move it out and it renders into the main window instead). The reverse link does not exist: nothing auto-associates a consumer with a viewport's texture, so anything that samples the height buffer (the map ColorRects, each via a `ViewportTexture` bound in the inspector) must reference the SubViewport explicitly. With multiple map SubViewports each renders independently from the camera nested inside it, with no crosstalk, which is why a consumer cannot guess its source and has to be told. Documented under Godot's "Using Viewports" tutorial.
...
- The compositor's segment texture is a `RenderingDevice` texture, not a Godot `Texture2D`. To let a canvas shader sample it, the compositor takes a `Texture2DRD` resource via its `SegmentTexture` property and sets `SegmentTexture.TextureRdRid = _segments` when it (re)creates the RD texture. That same `Texture2DRD` `.tres` is bound to each material's `segments` uniform in the inspector, so the compositor and the consumers share one resource with no script wiring it. Both run on the same `RenderingServer.GetRenderingDevice()`, so the RID is valid for sampling.

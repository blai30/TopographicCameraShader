...
- `scripts/MapUi.cs`: orchestration. Holds the two map `ColorRect`s (minimap, world map) and binds the shader runtime inputs directly: at `_Ready` it sets each one's `height_buffer` to the `MapView` viewport texture and `segments` to the compositor's `SegmentTexture` (via the `MapCompositor` export) through its private `BindTextures` helper; it owns the pan/zoom window state and pushes the window into each map's `window_center`/`window_span`/`px_per_uv` via `SetWindow` (which computes `px_per_uv = view.Size.X / span` so the line width is constant in screen pixels) and updates the two marker overlays each frame. The marker heading comes from the player's yaw (`MarkerRotation()`, currently `-Player.GlobalRotation.Y`; flip the sign if the arrow points backward).

...
- **First-frame "Uniforms were never supplied for set (1)".** A consumer can draw before the compositor's first render-once, sampling an empty RID. Both layers required: (1) `volatile bool HasProduced`, gate consumer visibility on it (removing it re-errors the game every run); (2) placeholder RID armed at load via `BufferSize` pre-sizing, shader treats `seg.x <= 0` as no-contour.
- **`BufferSize` = SubViewport size (demo `2048x2048`).** Else the first render resizes and reassigns the published RID, tripping "set (1)" once on a cold editor scene open (Godot #118292). Editor-only and benign; the game runs clean.
